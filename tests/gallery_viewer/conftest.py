...
specific scenario shape (number of dates, versions, etc.). Those local fixtures
...
def make_gallery_dir(
    ...
    *,
    ...
    with_data: if True, write a CSV per date.
    with_plots: if True, write a fake PNG for v1 of each date.
    ...
    for date, n_versions in dates_versions.items():
        if with_data:
            ...
            df.to_csv(d / "data" / f"data_{date}.csv", index=False)
        if with_plots:
            (d / "plots" / f"plot_{date}_v1.png").write_bytes(b"\x89PNG fake")
...
def gallery_with_chain(tmp_path):
    """A single-backend Gallery with one date and v1, v2, v3 — for diff/chain tests."""
